...
from postprocessing import posenet_postprocessing, posenet_draw_skel_and_kp, posenet_to_object

parser = argparse.ArgumentParser(description='PoseNet demo on GPU/NPU')
...
interpreter = Interpreter(
    model_path='posenet_mobilenet.tflite',
...
try:
    for frames_by_sink, marks in gst_grouped_frames(input_pipeline):
        ...
        with mark_performance('inference_done', marks):
            ...
            interpreter.set_tensor(input_details[0]['index'], rgb_numpy_arr_to_input_tensor(interpreter, arr=frame))
            interpreter.invoke()

        with mark_performance('postprocessing_done', marks):
            # Get prediction (dequantized)
            pose_scores, keypoint_scores, keypoint_coords = posenet_postprocessing(interpreter)
            poses = posenet_to_object(pose_scores, keypoint_scores, keypoint_coords)
            print(f'    Number of poses: {len(poses)}')

        # Draw overlay
        posenet_draw_skel_and_kp(
            frame,
            pose_scores,
            keypoint_scores,
            keypoint_coords,
